...
export class EmailService {
  ...
  constructor() {
    ...
    // Final inbox receiving contact form messages
    this.recipientEmail =
      process.env.CONTACT_EMAIL || smtpUser || "info@weare4s.com";

    if (!smtpUser || !smtpPassword) {
      console.error("[EmailService] Missing SMTP_USER or SMTP_PASSWORD.");
      return;
    }
    ...
    this.transporter.verify((err, success) => {
      ...
      this.isConfigured = true;
      console.log("[EmailService] SMTP is ready. Emails will be sent to:", this.recipientEmail);
    });
  }
  ...
  async sendContactEmail(data: ContactMessage): Promise<boolean> {
    if (!this.isConfigured || !this.transporter) {
      console.error("[EmailService] Email NOT sent: SMTP not configured.");
      return false;
    }
    ...
    try {
      ...
      console.log("[EmailService] Contact email delivered to:", this.recipientEmail);
      return true;
    } catch (err: any) {
      console.error("[EmailService] Error sending contact email:", err.message);
      return false;
    }
  }

  // -----------------------------
  // Send confirmation email back to visitor
  // -----------------------------
  async sendConfirmationEmail(recipientEmail: string): Promise<boolean> {
    if (!this.isConfigured || !this.transporter) {
      console.error("[EmailService] Confirmation email NOT sent: SMTP not configured.");
      return false;
    }
    ...
    try {
      ...
      console.error("[EmailService] Error sending confirmation email:", err.message);
      return false;
    }
  }
}
